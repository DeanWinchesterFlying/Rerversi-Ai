...
class ReversiPlayer:
    ...
    def search_moves(self, me, enemy):
        ...
        for i in range(self.play_config.simulation_num_per_move):
            coroutine_list.append(self.start_do_simulation(me, enemy, is_root_node=True))

        coroutine_list.append(self.prediction_worker())
        loop.run_until_complete(asyncio.gather(*coroutine_list))

    async def start_do_simulation(self, me, enemy, is_root_node=True):
        ...
        with await self.sem:
            ...
            value = await self.do_simulation(env, is_root_node)
            self.running_simulation_num -= 1
            return value

    async def do_simulation(self, env, is_root_node=True):
        key = self.get_state_key(env)
        ...
        if key not in self.expanded:  # reach a leaf node
            value = await self.expand_and_evaluate(env)  # expand this node
        ...
        value = await self.do_simulation(env)
    ...
    async def expand_and_evaluate(self, env: ReversiEnv):
        key = self.get_state_key(env)
        another_side_key = self.get_state_key(env, another_side=True)
    ...
    async def prediction_worker(self):
        ...
        while self.running_simulation_num > 0 or margin > 0:
            ...
            policy_ary, value_ary = self.model.predict(data)  # shape=(N, 2, 8, 8)
